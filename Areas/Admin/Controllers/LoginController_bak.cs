...
{
    [Area("Admin")]
    public class LoginController_bak : Controller
    {
        readonly DependancyInjection DI;
        public LoginController_bak(DependancyInjection _dependancyInjection)
        {
        ...
        public void Update_ActiveUser(string UserName, string Password, bool IsADUser)
        {
            ...
            FormsAuthentication_bak.UpdateLoginInfo(DI.session, Convert.ToInt16(dtResult.Rows[0]["Code"]), UserName.ToString(),
        ...
        {
            ActiveUser av = FormsAuthentication_bak.GetCurrentUser(DI.session);
            if (av != null)
            {
                FormsAuthentication_bak.UpdateUserActivityLog(DI.session, "Logout", false, true, true, DI.dBAccess);
                DI.session.Clear();
            }
